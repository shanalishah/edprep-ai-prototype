"""
Production Essay Scorer for IELTS Writing Assessment
This module integrates the advanced trained models into the API
"""
...
class ProductionEssayScorer:
    """
    Production-ready essay scorer using advanced trained models
    """
    ...
    def _load_models(self):
        ...
        try:
            ...
            # Load production Random Forest model
            model_path = self.models_dir / "Production_Random_Forest_model.pkl"
            if model_path.exists():
                with open(model_path, 'rb') as f:
                    self.model = pickle.load(f)
            
            self.is_loaded = self.model is not None and self.scaler is not None and self.tfidf_vectorizer is not None
            
            if self.is_loaded:
                print("✅ Production ML models loaded successfully")
            else:
                print("⚠️  Production ML models not found, falling back to rule-based scoring")
                
        except Exception as e:
            print(f"❌ Error loading production ML models: {e}")
            self.is_loaded = False
    
    def extract_production_features(self, essay: str) -> np.ndarray:
        """Extract the same features used during production training"""
        if not essay or pd.isna(essay):
            return self._get_empty_production_features()
        ...
        features = np.array([
            ...
            sentence_length_std, vocabulary_richness, complex_sentences,
            linking_words, academic_words, comma_count, period_count,
            question_count, exclamation_count, semicolon_count, colon_count,
            capital_ratio, repetition_ratio, unique_word_ratio, hapax_ratio,
            avg_syllables_per_word, pronoun_ratio, argument_indicators,
            conclusion_indicators, opinion_indicators, example_indicators
        ])
    ...
    def _get_empty_production_features(self) -> np.ndarray:
        """Return empty features for null essays"""
        return np.zeros((1, 28))
    
    def create_tfidf_features(self, essay: str) -> np.ndarray:
        """Create TF-IDF features for a single essay"""
        if not essay or pd.isna(essay):
            return np.zeros((1, 500))  # Default TF-IDF size
        
        # Clean essay for TF-IDF
        essay = str(essay).lower()
        essay = re.sub(r'[^\w\s]', ' ', essay)
        essay = re.sub(r'\s+', ' ', essay)
        
        # Transform using the trained TF-IDF vectorizer
        tfidf_features = self.tfidf_vectorizer.transform([essay])
        return tfidf_features.toarray()
    
    def score_essay_production(self, essay: str, prompt: str = "", task_type: str = "Task 2") -> Dict[str, float]:
        """
        Score essay using production ML model
        """
        ...
        try:
            ...
            text_features = self.extract_production_features(essay)
            
            # Create TF-IDF features
            tfidf_features = self.create_tfidf_features(essay)
            
            # Combine features
            features = np.hstack([text_features, tfidf_features])
            ...
            print(f"Error in production ML scoring: {e}")
            return self._fallback_scoring(essay, prompt, task_type)
